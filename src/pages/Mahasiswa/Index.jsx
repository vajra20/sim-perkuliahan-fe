...
const Index = () => {
	...
	useEffect(() => {
		const fetchAllData = async () => {
			...
			const assignedTugas = MahasiswaData?.assignedTugas.map(
				(item) => item.statusTugas.statusTugas
			);

			const assignTasks = assignedTugas.filter(
				(item) => item === "Ditugaskan"
			);

			const submitTasks = assignedTugas.filter(
				(item) => item === "Diserahkan"
			);

			const undueTask = assignedTugas.filter(
				(item) => item === "Telat Diserahkan"
			);

			const unSubmitTask = assignedTugas.filter(
				(item) => item === "Belum Diserahkan"
			);
			...
			setTask((prev) => ({
				...prev,
				ditugaskan: assignTasks,
				diserahkan: submitTasks,
				telatDiSerahkan: undueTask,
				belumDiSerahkan: unSubmitTask,
			}));
		};
			...
			<div className="md:px-7 lg:py-6 android:px-3">
				<div className="lg:grid md:flex md:flex-col lg:gap-0 xl:gap-10 grid-cols-dashboard mb-2">
					<div className="md:px-0 lg:pl-0 lg:pr-10 android:px-0">
						...
						<div className="w-full flex justify-center sm:gap-10 android:gap-5 md:mb-5 sm:mb-0 android:mb-10 android:flex-row">
						...
						{/* <div className=" w-full sm:my-6 android:mb-10">
							<div className="bg-[#AC98F933] border border-dark-gray sm:rounded-3xl android:rounded-2xl sm:py-6 sm:px-8 android:py-4 android:px-6">
								<div className="flex items-center justify-between gap-2">
									<a
										target="_blank"
										href={`${apiUrl}/api/image/${mahasiswaById}`}
										className="text-dashboard-line sm:text-2xl android:text-base sm:font-medium android:font-bold text-center"
									>
